...
import { auth, onAuth, getSubscription, type SubscriptionDoc, logout } from "@/lib/firebase"
...
export default function SiteInspector() {
  ...
  const checkValid = (doc: SubscriptionDoc | null) => {
    if (!doc?.expirationAt) return false
    const exp = doc.expirationAt.toDate().getTime()
    return exp > Date.now()
  }
  ...
  const fetchSubscription = async () => {
    ...
    try {
      ...
      setSubscription(sub)
      setIsSubscriptionValid(checkValid(sub))
    } catch {
  ...
  // Auto-redirect if logged-in but invalid subscription
  useEffect(() => {
    if (!isAuthReady) return
    if (user && !isCheckingSubscription && !isSubscriptionValid) {
      if (window.location.pathname !== "/subscription") router.replace("/subscription")
    }
  }, [isAuthReady, user, isCheckingSubscription, isSubscriptionValid, router])
  ...
  const handleAnalyzeClick = async () => {
    ...
    if (!isSubscriptionValid) return redirectToSubscription()
    await analyzeSite()
  }
...
<script>${safeFullJS}</script>
      ...
      return (
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="outline" className="h-[32px] rounded-[13px]">
  ...
  const inputPlaceholder = !user
    ? "Sign in to analyze (you'll be redirected)"
    : !isSubscriptionValid
      ? "Your trial has ended. Go to subscription"
      : "https://example.com"

  const onInputFocus = () => {
    if (!user) router.push("/auth")
    else if (!isSubscriptionValid) router.push("/subscription")
  }
    ...
    <div className="min-h-screen bg-white overflow-hidden p-4 sm:p-8">
      ...
      <div className="max-w-4xl mx-auto p-6 sm:p-10 pb-20">
        <div className="text-center mb-10">
          ...
          {user && !isSubscriptionValid && !isCheckingSubscription && (
            <div className="mt-4 text-sm text-red-700 bg-red-50 border border-red-200 rounded-md px-3 py-2">
              Your trial has ended or is invalid. Please renew your subscription to continue.
            </div>
          )}
        </div>
          ...
          <div className="h-full w-full bg-[#fff] ring-4 ring-[#FAFAFA] rounded-[12px] flex items-center p-1 ">
            ...
            <input
              ...
              className={`flex-grow h-full bg-transparent text-black focus:outline-none focus:ring-0 placeholder-[#888] text-sm ${
                user && isSubscriptionValid ? "" : "cursor-pointer"
              }`}
              readOnly={user ? !isSubscriptionValid : true}
            />
            <button
