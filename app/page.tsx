...
// --- AI BUILDER: Import de la librairie demandée ---
...
export default function SiteInspector() {
  ...
  const handleInitiateAIStudio = async () => {
    ...
    try {
        const GEMINI_API_KEY = "AIzaSyDQRPehpA6TVODVOcofx7NCQr7vhTnk6zM";
        ...
        const response = await generativeModel.generateContentStream({ contents, generationConfig: config });
        ...
        for await (const chunk of response.stream) {
            ...
            const conversationalPart = streamedResponse.split(/<file name="/)[0];
            setAiMessage(conversationalPart);

            const htmlMatch = streamedResponse.match(/<file name="index.html">([\s\S]*?)<\/file>/);
            const cssMatch = streamedResponse.match(/<file name="style.css">([\s\S]*?)<\/file>/);
            const jsMatch = streamedResponse.match(/<file name="script.js">([\s\S]*?)<\/file>/);
            
            setAiGeneratedFiles({
                html: htmlMatch ? htmlMatch[1].trim() : aiGeneratedFiles.html,
                css: cssMatch ? cssMatch[1].trim() : aiGeneratedFiles.css,
                js: jsMatch ? jsMatch[1].trim() : aiGeneratedFiles.js
            });
        }
    } catch (err) {
        console.error("Erreur Gemini:", err);
        setAiMessage("Désolé, une erreur est survenue. Vérifiez la clé API et la console.");
    } finally {
