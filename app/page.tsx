...
// NOUVEAU TYPE POUR LE RESULTAT REDUIT
...
export default function SiteInspector() {
  ...
  // NOUVEAUX ETATS POUR LE PROMPT REDUCER
  ...
  const generateWithGemini = async (userInput: string) => {
    if (!result) return

    setChatLoading(true)
    try {
      const ai = new GoogleGenAI({
        apiKey: process.env.NEXT_PUBLIC_GEMINI_API_KEY || firebaseConfig.apiKey,
      })
      ...
      const prompt = `Based on the extracted website design, create separate HTML, CSS, and JS files for: ${userInput}
...
Please generate:
1. A complete index.html file
2. A complete styles.css file  
3. A complete script.js file

Make sure the files are functional and properly linked together.`
      ...
      setChatMessages((prev) => [
        ...
        {
          role: "assistant",
          content: "Sorry, there was an error generating the files. Please try again.",
        },
      ])
    } finally {
  ...
  const analyzeSite = async (urlToAnalyze = url) => {
    ...
    setError(null)
    setResult(null)
    // NOUVEAU : Réinitialiser le résultat réduit
...
${result.fullJS}
  ...
  const buildSeparateFilesPrompt = (): string => {
    // MODIFICATION: Utilise le code réduit si le réducteur est actif
    const source = isReducerActive ? reducedResult : result
    if (!source) return ""

    const libs =
      result && result.requiredCdnUrls && result.requiredCdnUrls.length
...
<body>
  ...
  // NOUVEL EFFET POUR DÉCLENCHER LA RÉDUCTION DU CODE
  useEffect(() => {
    if (!result) return

    const reduceCode = () => {
      // 1. Réduire le JS (uniquement les scripts d'animation)
      const animationJs = result.animationFiles
        .filter((file) => file.type === "js")
        .map((file) => `// Script d'animation extrait de: ${file.url}\n${file.content}`)
        .join("\n\n// ===== SCRIPT SUIVANT =====\n\n")

      // 2. Réduire le HTML et le CSS
      const { reducedHtml, reducedCss } = reduceHtmlAndCss(result.fullHTML, result.fullCSS)

      setReducedResult({
        ...result,
        fullHTML: reducedHtml,
        fullCSS: reducedCss,
        fullJS: animationJs,
      })
    }
  ...
  // NOUVELLE FONCTION : LOGIQUE DE RÉDUCTION HTML ET CSS
  const reduceHtmlAndCss = (html: string, css: string): { reducedHtml: string; reducedCss: string } => {
    const parser = new DOMParser()
    const doc = parser.parseFromString(html, "text/html")

    // Partie 1: Remplace les SVG par des Emojis
    doc.querySelectorAll("svg").forEach((svg) => {
      const emojiSpan = doc.createElement("span")
      emojiSpan.textContent = "🎨" // Emoji de substitution
      emojiSpan.setAttribute("role", "img")
      emojiSpan.setAttribute("aria-label", "icon")
      svg.parentNode?.replaceChild(emojiSpan, svg)
    })
    const reducedHtml = doc.body.innerHTML

    // Partie 2: Extrait les classes CSS utilisées
    const usedClasses = new Set<string>()
    doc.querySelectorAll("*").forEach((el) => {
      el.classList.forEach((cls) => usedClasses.add(cls))
    })

    // Partie 3: Filtre le CSS
    // Attention: ce filtre est une simplification et peut ne pas fonctionner avec des sélecteurs très complexes.
    ...
    const a_garder_css = cssRules.filter((rule) => {
      ...
      // Exclut les sélecteurs de balises génériques
      const isGenericTag = /^(html|body|\*|div|p|h[1-6]|a|span|section|main|header|footer|nav)\b/i.test(selectorPart)
      if (isGenericTag) return false

      // Garde la règle si elle contient une des classes utilisées
      return Array.from(usedClasses).some((usedClass) =>
        // Vérifie si la classe est présente, précédée d'un '.'
        new RegExp(`\\.${usedClass}(\\s|\\.|:|,)`, "g").test(selectorPart + " "),
      )
    })
    ...
    <div className="min-h-screen bg-white overflow-hidden p-4 sm:p-8">
      ...
      <div className="max-w-4xl mx-auto p-6 sm:p-10 pb-20">
        ...
        {result && (
          <div className="space-y-12">
            <div>
              <h3 className="text-2xl font-bold text-black mb-4">Aperçu de l'interface utilisateur (complet)</h3>
              <iframe
                title="UI preview"
            ...
            {/* NOUVEL IFRAME POUR LA VERSION RÉDUITE */}
            {reducedResult && (
              <div>
                <h3 className="text-2xl font-bold text-black mb-4">Aperçu avec "Prompt Reducer"</h3>
                <p className="text-sm text-gray-500 mb-3">
                  Version optimisée avec SVG remplacés par des emojis, et code JS/CSS réduit aux animations et styles
                  utilisés.
                </p>
                <iframe
                  title="Aperçu Réduit"
        ...
        <div className="mt-8 max-w-4xl mx-auto">
          <div className="bg-white rounded-lg border border-gray-200 shadow-sm">
            <div className="p-4 border-b border-gray-200">
              <h3 className="text-lg font-semibold">AI Chat - Generate Custom Files</h3>
              <p className="text-sm text-gray-600">
                Ask the AI to create specific files based on your extracted design
              </p>
            </div>
            ...
            <div className="h-96 overflow-y-auto p-4 space-y-4">
              {chatMessages.length === 0 ? (
                <div className="text-center text-gray-500 py-8">
                  <p>Start a conversation to generate custom HTML, CSS, and JS files</p>
                  <p className="text-sm mt-2">Example: "Create a responsive landing page" or "Make a contact form"</p>
                </div>
              ) : (
            ...
            <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-white rounded-lg border border-gray-200">
                <div className="p-3 border-b border-gray-200 flex justify-between items-center">
                  <h4 className="font-medium">index.html</h4>
                  <Button size="sm" variant="outline" onClick={() => copyToClipboard(generatedFiles.html, "html-file")}>
                    Copy
                  </Button>
                </div>
                <pre className="p-3 text-xs overflow-auto max-h-64 bg-gray-50">
              ...
              <div className="bg-white rounded-lg border border-gray-200">
                <div className="p-3 border-b border-gray-200 flex justify-between items-center">
                  <h4 className="font-medium">styles.css</h4>
                  <Button size="sm" variant="outline" onClick={() => copyToClipboard(generatedFiles.css, "css-file")}>
                    Copy
                  </Button>
                </div>
                <pre className="p-3 text-xs overflow-auto max-h-64 bg-gray-50">
              ...
              <div className="bg-white rounded-lg border border-gray-200">
                <div className="p-3 border-b border-gray-200 flex justify-between items-center">
                  <h4 className="font-medium">script.js</h4>
                  <Button size="sm" variant="outline" onClick={() => copyToClipboard(generatedFiles.js, "js-file")}>
                    Copy
                  </Button>
                </div>
                <pre className="p-3 text-xs overflow-auto max-h-64 bg-gray-50">
                  <code>{generatedFiles.js}</code>
                </pre>
              </div>
            </div>
          )}
      ...
      {result && (
        <div className="fixed bottom-0 left-0 right-0 p-4 flex justify-center z-50">
          <div className="flex items-center gap-4 bg-white/70 backdrop-blur-lg p-2 rounded-xl shadow-lg border border-gray-200">
            {/* NOUVEAU BOUTON PROMPT REDUCER */}
            <div className="flex items-center space-x-2 pl-2">
              <Switch id="prompt-reducer" checked={isReducerActive} onCheckedChange={setIsReducerActive} />
              <Label htmlFor="prompt-reducer" className="text-sm font-medium whitespace-nowrap">
                Prompt Reducer
              </Label>
            </div>
            ...
            <Button
              ...
              variant="outline"
              className="h-[40px] rounded-[12px] text-sm font-medium px-4 bg-white"
            >
